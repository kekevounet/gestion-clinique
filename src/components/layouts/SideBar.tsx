...
import type { SideBarProps } from '../common/type'
...
export default function SideBar({ sideOuvert, setSideOuvert }: SideBarProps)
{
  // Déclaration
            ...
            <div className="w-full flex flex-col" key={index}>
              {menu.sousMenu
                ...
                :
                  <NavLink
                    to={`/${menu.lien}`}
                    onClick={() => { if(isMobile){ setSideOuvert(false)}}}
              ...
              {menu.sousMenu && openMenu === index && (
                <div className="flex flex-col duration-500">
                  {menu.sousMenu.map((subMenu,subIndex)=>(
                    <NavLink to={`/${subMenu.lien}`} key={subIndex} onClick={() => { if(isMobile){ setSideOuvert(false)}}} className={({ isActive })=> `w-full h-12 text-base flex items-center space-x-5 cursor-pointer group relative z-30 ${theme === 'clair' ? 'bg-zinc-100 ' : 'bg-zinc-900'} ${isActive ? `border-(--couleur) border-r-4` : '' } `} >
